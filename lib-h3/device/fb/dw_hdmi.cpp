...
static const u16 kCscCoeffDefault[3][4] = {{0x2000, 0x0000, 0x0000, 0x0000}, {0x0000, 0x2000, 0x0000, 0x0000}, {0x0000, 0x0000, 0x2000, 0x0000}};
static const u16 kCscCoeffRgbInEitu601[3][4] = {{0x2591, 0x1322, 0x074b, 0x0000}, {0x6535, 0x2000, 0x7acc, 0x0200}, {0x6acd, 0x7534, 0x2000, 0x0200}};
static const u16 kCscCoeffRgbOutEitu601[3][4] = {{0x2000, 0x6926, 0x74fd, 0x010e}, {0x2000, 0x2cdd, 0x0000, 0x7e9a}, {0x2000, 0x0000, 0x38b4, 0x7e3b}};

static void DwHdmiWrite(struct dw_hdmi* hdmi, u8 val, int offset)
{
...
static u8 DwHdmiRead(struct dw_hdmi* hdmi, int offset)
{
    switch (hdmi->reg_io_width)
    {
        case 1:
...
static u8 (*hdmi_read)(struct dw_hdmi* hdmi, int offset) = DwHdmiRead;
static void (*hdmi_write)(struct dw_hdmi* hdmi, u8 val, int offset) = DwHdmiWrite;

static void HdmiMod(struct dw_hdmi* hdmi, unsigned reg, u8 mask, u8 data)
{
...
static bool HdmiBusFmtIsRgb(unsigned int bus_format)
{
    switch (bus_format)
    {
...
static bool HdmiBusFmtIsYuv444(unsigned int bus_format)
{
    switch (bus_format)
    {
...
static bool HdmiBusFmtIsYuv422(unsigned int bus_format)
{
    switch (bus_format)
    {
...
static int HdmiBusFmtColorDepth(unsigned int bus_format)
{
    switch (bus_format)
    {
        case MEDIA_BUS_FMT_RGB888_1X24:
...
static int IsColorSpaceDecimation(struct dw_hdmi* hdmi)
{
    if (!HdmiBusFmtIsYuv422(hdmi->hdmi_data.enc_out_bus_format)) return 0;

    if (HdmiBusFmtIsRgb(hdmi->hdmi_data.enc_in_bus_format) || HdmiBusFmtIsYuv444(hdmi->hdmi_data.enc_in_bus_format)) return 1;

    return 0;
}

static void HdmiAvComposer(struct dw_hdmi* hdmi, const struct display_timing* edid)
{
    ...
    inv_val |= (edid->flags & DISPLAY_FLAGS_VSYNC_HIGH ? HDMI_FC_INVIDCONF_VSYNC_IN_POLARITY_ACTIVE_HIGH : HDMI_FC_INVIDCONF_VSYNC_IN_POLARITY_ACTIVE_LOW);

    inv_val |= (edid->flags & DISPLAY_FLAGS_HSYNC_HIGH ? HDMI_FC_INVIDCONF_HSYNC_IN_POLARITY_ACTIVE_HIGH : HDMI_FC_INVIDCONF_HSYNC_IN_POLARITY_ACTIVE_LOW);

    inv_val |= (mdataenablepolarity ? HDMI_FC_INVIDCONF_DE_IN_POLARITY_ACTIVE_HIGH : HDMI_FC_INVIDCONF_DE_IN_POLARITY_ACTIVE_LOW);

    inv_val |= (edid->hdmi_monitor ? HDMI_FC_INVIDCONF_DVI_MODEZ_HDMI_MODE : HDMI_FC_INVIDCONF_DVI_MODEZ_DVI_MODE);
...
static int IsColorSpaceConversion(struct dw_hdmi* hdmi)
{
    return hdmi->hdmi_data.enc_in_bus_format != hdmi->hdmi_data.enc_out_bus_format;
}

static void DwHdmiUpdateCscCoeffs(struct dw_hdmi* hdmi)
{
    const u16(*csc_coeff)[3][4] = &kCscCoeffDefault;
    unsigned int i;
    u32 csc_scale = 1;

    if (IsColorSpaceConversion(hdmi))
    {
        if (HdmiBusFmtIsRgb(hdmi->hdmi_data.enc_out_bus_format))
        {
            csc_coeff = &kCscCoeffRgbOutEitu601;
        }
        else if (HdmiBusFmtIsRgb(hdmi->hdmi_data.enc_in_bus_format))
        {
            csc_coeff = &kCscCoeffRgbInEitu601;
            csc_scale = 0;
        }
    }

    /* The CSC registers are sequential, alternating MSB then LSB */
    for (i = 0; i < ARRAY_SIZE(kCscCoeffDefault[0]); i++)
    {
    ...
    HdmiMod(hdmi, HDMI_CSC_SCALE, HDMI_CSC_SCALE_CSCSCALE_MASK, csc_scale);
}

static int IsColorSpaceInterpolation(struct dw_hdmi* hdmi)
{
    if (!HdmiBusFmtIsYuv422(hdmi->hdmi_data.enc_in_bus_format)) return 0;

    if (HdmiBusFmtIsRgb(hdmi->hdmi_data.enc_out_bus_format) || HdmiBusFmtIsYuv444(hdmi->hdmi_data.enc_out_bus_format)) return 1;

    return 0;
}

static void HdmiVideoCsc(struct dw_hdmi* hdmi)
{
    int color_depth = 0;
    ...
    switch (HdmiBusFmtColorDepth(hdmi->hdmi_data.enc_out_bus_format))
    {
        case 8:
    ...
    HdmiMod(hdmi, HDMI_CSC_SCALE, HDMI_CSC_SCALE_CSC_COLORDE_PTH_MASK, color_depth);

    DwHdmiUpdateCscCoeffs(hdmi);
}

/* hdmi initialization step b.4 */
static void HdmiEnableVideoPath(struct dw_hdmi* hdmi, bool audio)
{
    uint clkdis;
...
static void HdmiClearOverflow(struct dw_hdmi* hdmi)
{
    uint val, count;
    ...
    for (count = 0; count < 4; count++) hdmi_write(hdmi, val, HDMI_FC_INVIDCONF);
}

static void HdmiVideoPacketize(struct dw_hdmi* hdmi)
{
    ...
    HdmiMod(hdmi, HDMI_VP_CONF, HDMI_VP_CONF_PR_EN_MASK | HDMI_VP_CONF_BYPASS_SELECT_MASK, vp_conf);
    ...
    HdmiMod(hdmi, HDMI_VP_CONF, HDMI_VP_CONF_BYPASS_EN_MASK | HDMI_VP_CONF_PP_EN_ENMASK | HDMI_VP_CONF_YCC422_EN_MASK, vp_conf);

    HdmiMod(hdmi, HDMI_VP_STUFF, HDMI_VP_STUFF_PP_STUFFING_MASK | HDMI_VP_STUFF_YCC422_STUFFING_MASK,
             HDMI_VP_STUFF_PP_STUFFING_STUFFING_MODE | HDMI_VP_STUFF_YCC422_STUFFING_STUFFING_MODE);

    HdmiMod(hdmi, HDMI_VP_CONF, HDMI_VP_CONF_OUTPUT_SELECTOR_MASK, output_select);
}
 ...
 *			pin{15~8}  <==> b[7:0]
 */
static void HdmiVideoSample(struct dw_hdmi* hdmi)
{
    u32 color_format;
    uint val;
    ...
    val = HDMI_TX_INVID0_INTERNAL_DE_GENERATOR_DISABLE | ((color_format << HDMI_TX_INVID0_VIDEO_MAPPING_OFFSET) & HDMI_TX_INVID0_VIDEO_MAPPING_MASK);

    hdmi_write(hdmi, val, HDMI_TX_INVID0);

    /* enable tx stuffing: when de is inactive, fix the output data to 0 */
    val = HDMI_TX_INSTUFFING_BDBDATA_STUFFING_ENABLE | HDMI_TX_INSTUFFING_RCRDATA_STUFFING_ENABLE | HDMI_TX_INSTUFFING_GYDATA_STUFFING_ENABLE;
...
int dw_hdmi_enable(struct dw_hdmi* hdmi, const struct display_timing* edid)
{
    ...
    HdmiAvComposer(hdmi, edid);

    ret = hdmi->phy_set(hdmi, edid->pixelclock.typ);

    if (ret) return ret;

    HdmiEnableVideoPath(hdmi, edid->hdmi_monitor);

    HdmiVideoPacketize(hdmi);
    HdmiVideoCsc(hdmi);
    HdmiVideoSample(hdmi);

    HdmiClearOverflow(hdmi);
